...
class RagServiceServicer(rag_pb2_grpc.RagServiceServicer):
    ...
    async def Query(self, request, context):
        ...
        try:
            ...
            if is_chroma_up:
                context_text = "The requirement is to provide a comprehensive response focusing on security and local processing."
                
            prompt = f"Based on the following context, answer the query.\nContext: {context_text}\nQuery: {query}"
            
            # Select first available Ollama model
            selected_model = "llama3.1:8b"
            is_pulled = await ollama.is_model_pulled(selected_model)
            if not is_pulled:
                # Try fallback names
                is_pulled = await ollama.is_model_pulled("llama3")
                if is_pulled:
                    selected_model = "llama3"
                else:
                    is_pulled = await ollama.is_model_pulled("mistral")
                    if is_pulled:
                        selected_model = "mistral"
                    else:
                        selected_model = "llama3.1:8b" # default fallback
