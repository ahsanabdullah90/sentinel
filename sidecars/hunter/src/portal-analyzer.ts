...
export async function analyzePortal(url: string): Promise<PortalViabilityReport> {
  console.log(
    JSON.stringify({ event: 'progress', portalId: 'detector', message: `Analyzing ${url} with Gemini...` })
  );
  ...
  try {
    ...
    if (apiKey) {
      console.log(
        JSON.stringify({ event: 'progress', portalId: 'detector', message: 'Calling Gemini API for intelligent detection...' })
      );
      ...
      try {
        console.log(
          JSON.stringify({ event: 'progress', portalId: 'detector', message: 'Trying gemini-2.5-flash-lite...' })
        );
        ...
        try {
          console.log(
            JSON.stringify({ event: 'progress', portalId: 'detector', message: 'Failed with lite. Trying gemini-2.5-flash...' })
          );
          const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });
          result = await model.generateContent(prompt);
        } catch (err2) {
          console.log(
            JSON.stringify({ event: 'progress', portalId: 'detector', message: 'Failed with 2.5-flash. Falling back to gemini-1.5-flash...' })
          );
      ...
      console.log(
        JSON.stringify({ event: 'progress', portalId: 'detector', message: 'No GEMINI_API_KEY found, falling back to basic heuristics...' })
      );
    }
    ...
    if (!searchSelector) {
      console.log(
        JSON.stringify({ event: 'progress', portalId: 'detector', message: 'Running heuristic selector detection...' })
      );
