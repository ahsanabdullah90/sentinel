...
function App() {
  ...
  const [settings, setSettings] = useState({ ollamaModel: 'phi3', ollamaUrl: 'http://127.0.0.1:11434' });
  ...
  async function finishActiveHunt(portalId: string) {
    try {
      ...
      const countRes = await db.select<any[]>('SELECT COUNT(*) as cnt FROM opportunities WHERE portal_id = ?', [portalId]);
      const oppCount = countRes[0]?.cnt || 0;
      
      const portal = portals.find(p => p.id === portalId);
      const renderingMode = portal?.selector_config || portalId === 'xbfs76tfq' ? 'Browser (Playwright)' : 'Static HTML';
  ...
  const triggerNextQueuePortal = async () => {
    ...
    if (nextIndex < queueRef.current.length) {
      ...
      setHuntLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] Queue progress: Starting next website hunt (${nextIndex + 1}/${queueRef.current.length})...`]);
      ...
      setTimeout(async () => {
        try {
          ...
          if (portal) {
            ...
            const id = await invoke('start_hunt_session', { portalId: nextPortalId, config: JSON.stringify(mappedPortal) });
          ...
          setHuntLogs(prev => [...prev, `[ERROR] Failed to start next website hunt: ${err instanceof Error ? err.message : String(err)}`]);
          void triggerNextQueuePortal();
        }
      }, 2000);
    } else {
      ...
      setHunting(false);
      setHuntLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] All sequential website hunts completed successfully!`]);
    }
  };
  ...
  async function handleStartHunt(portalId: string) {
    try {
      setHunting(true);
      setHuntLogs([`[${new Date().toLocaleTimeString()}] Spawning headless Chromium browser engine...`]);
      startTimeRef.current = Date.now();
      ...
      setHunting(false);
      setHuntLogs(prev => [...prev, `[ERROR] Failed to start hunt: ${error instanceof Error ? error.message : String(error)}`]);
    }
  }
    ...
    try {
      ...
      if (editingPortal) {
        ...
        await db.execute(
          ...
          [portal.id, portal.name, portal.url, portal.keywords, 'Active', 'public', 'generic_search']
        );
    ...
    <div className="app-container">
      ...
      <aside className="sidebar">
        ...
        <nav className="nav-links">
          ...
          <a href="#" className="nav-link" onClick={() => { setIsSettingsOpen(true); }}>
      ...
      <main className="main-content">
        ...
        {hunting && (
          <div className="card glass" style={{ marginTop: '15px', border: '1px solid rgba(0, 240, 255, 0.25)', padding: '15px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
              <div className="pulse-dot" style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#00f0ff', boxShadow: '0 0 8px #00f0ff' }}></div>
              <h4 style={{ margin: 0, color: '#00f0ff', fontSize: '0.85rem', letterSpacing: '0.05em', fontWeight: 'bold' }}>LIVE SCOUTING CONSOLE</h4>
            </div>
            <div 
              style={{
        ...
        <div className="card glass">
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', textAlign: 'left' }}>
              ...
              {schedulerInfo && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.78rem', color: '#a0aec0' }}>
                  <span className="pulse-dot" style={{ width: '6px', height: '6px', backgroundColor: schedulerInfo.includes('Active') ? '#39ff14' : '#8b90a0', boxShadow: schedulerInfo.includes('Active') ? '0 0 6px #39ff14' : 'none' }}></span>
              ...
              <button 
                ...
                onClick={() => {
                  ...
                  if (activePortals.length > 0) {
                    ...
                    setHuntLogs([`[${new Date().toLocaleTimeString()}] No active portals found to hunt.`]);
                  }
                }}
                disabled={hunting}
                style={{
                ...
                {portals.map((p) => {
                  ...
                  return (
                    <tr key={p.id}>
                      <td>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            style={{ fontSize: '0.8rem', color: '#8b90a0', textDecoration: 'none' }}
                          >
                            {p.url}
                          </a>
                      ...
                      <td>
                        <span style={{ fontSize: '0.85rem', color: '#e5e2e3' }}>{p.keywords || 'None'}</span>
                      </td>
                      <td>
                        {searchSelector ? (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            ...
                            <code style={{ fontSize: '0.75rem', color: '#8b90a0', fontFamily: 'monospace' }}>
                              {searchSelector}
                            </code>
                          </div>
                        ) : (
                      ...
                      <td>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.8rem', color: '#8b90a0' }}>
                            <span>Last Crawl:</span>
                            <span style={{ color: '#fff', fontWeight: 500 }}>
                              {p.last_run_at ? p.last_run_at : 'Never'}
                            </span>
                            {p.last_run_duration_ms ? (
                              <span style={{ color: 'var(--accent-color)', fontSize: '0.75rem', fontWeight: 600 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          <label className="switch">
                            <input
                              type="checkbox"
                              checked={isActive}
                              onChange={() => { void handleTogglePortalStatus(p.id, p.status || 'Active'); }}
                            />
                            <span className="slider"></span>
                          </label>
                          <span
                            className={isActive ? 'badge badge-success' : 'badge badge-secondary'}
                        ...
                        <div style={{ display: 'flex', gap: '5px' }}>
                          {!hunting ? (
                            <button
                              ...
                              onClick={() => { void triggerQueueHunt([p.id]); }}
                              disabled={!isActive}
                              style={{ opacity: isActive ? 1 : 0.5, cursor: isActive ? 'pointer' : 'not-allowed' }}
                              title={isActive ? 'Start hunting opportunities' : 'Activate portal to start hunt'}
                            >
                              <Play size={12} />
                            </button>
                          ) : (
                            <button className="btn btn-sm btn-danger" onClick={() => { void handleStopHunt(); }}>
                              <Square size={12} />
                            </button>
                          )}
                          <button
                      ...
                      <td>
                        <button
                          ...
                          onClick={async () => {
                            try {
                              const db = await SqlDatabase.load('sqlite:sentinel.db');
                              await db.execute("UPDATE opportunities SET status = 'drafted' WHERE id = ?", [o.id]);
                              void loadOpportunities();
                            } catch (e) {
                              console.error(e);
                            }
                          }}
                          disabled={o.status === 'drafted'}
                        >
                          {o.status === 'drafted' ? 'Draft Ready' : 'Generate Draft'}
                        </button>
                      </td>
                    </tr>
                  ))
